...
if (!defined('WP_GRAPHQL_GUTENBERG_PREVIEW_POST_TYPE_NAME')) {
    define(
        'WP_GRAPHQL_GUTENBERG_PREVIEW_POST_TYPE_NAME',
        'wgg_preview'
    );
}
...
if (!defined('WP_GRAPHQL_GUTENBERG_PREVIEW_GRAPHQL_PLURAL_NAME')) {
    define(
        'WP_GRAPHQL_GUTENBERG_PREVIEW_GRAPHQL_PLURAL_NAME',
        'BlockEditorPreviews'
    );
}
...
class BlockEditorPreview
{
    ...
    public static function insert_preview($post_id, $preview_post_id, $blocks, $registry)
    {
        $insert_options = [
            'post_title' => $post_id,
            'post_type' => WP_GRAPHQL_GUTENBERG_PREVIEW_POST_TYPE_NAME,
        ...
        $result = wp_insert_post($insert_options, true);
    ...
    public function __construct()
    {
        ...
        add_filter('graphql_RootQueryTo' . WP_GRAPHQL_GUTENBERG_PREVIEW_GRAPHQL_SINGLE_NAME . 'ConnectionWhereArgs_fields', function ($fields, $type) {
        ...
        add_filter('graphql_post_object_connection_query_args', function ($query_args, $source, $args, $context, $info) {
            if ($info->fieldName === lcfirst(WP_GRAPHQL_GUTENBERG_PREVIEW_GRAPHQL_PLURAL_NAME)) {
                if (isset($args['where']['previewedDatabaseId'])) {
                    if (!isset($query_args['meta_query'])) {
                        $query_args['meta_query'] = [];
                    }

                    $query_args['meta_query'][] = ['key' => 'post_id', 'value' => $args['where']['previewedDatabaseId']];
                }
